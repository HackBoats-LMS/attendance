...
export default function LoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        // Show full-page overlay and keep it until the new page loads
        setNavigating(true);
        router.push("/dashboard");
        // Don't call setLoading(false) — overlay stays until navigation completes
        return;
      } else {
  ...
  return (
    <>
      {/* Full-page loading overlay shown after successful login */}
      {navigating && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-bg gap-6 animate-fade-in">
          ...
          <div className="text-center">
            ...
            <p className="text-ink-muted text-xs mt-1">Loading your dashboard</p>
          </div>
        </div>
      )}
        ...
        <div className="card w-full max-w-md p-8 sm:p-10">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <button
              id="login-submit"
              type="submit"
              className="btn-primary w-full mt-2 flex items-center justify-center gap-2 h-11 relative overflow-hidden"
              disabled={loading}
            >
              {loading ? (
                <>
